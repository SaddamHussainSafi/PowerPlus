...
class Class_Widget_Reset_Password extends Class_Abstract_Form_Widget {
	...
	public function get_title() { return esc_html__( 'PowerPlus Reset Password', 'powerplus-toolkit' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'reset_content', array( 'label' => esc_html__( 'Fields', 'powerplus-toolkit' ) ) );
		$this->add_control( 'password_label', array( 'label' => esc_html__( 'Password Label', 'powerplus-toolkit' ), 'type' => Controls_Manager::TEXT, 'default' => esc_html__( 'New Password', 'powerplus-toolkit' ) ) );
		$this->add_control( 'confirm_label', array( 'label' => esc_html__( 'Confirm Label', 'powerplus-toolkit' ), 'type' => Controls_Manager::TEXT, 'default' => esc_html__( 'Confirm Password', 'powerplus-toolkit' ) ) );
		$this->add_control( 'button_text', array( 'label' => esc_html__( 'Button Text', 'powerplus-toolkit' ), 'type' => Controls_Manager::TEXT, 'default' => esc_html__( 'Set New Password', 'powerplus-toolkit' ) ) );
		$this->add_control( 'show_strength', array( 'label' => esc_html__( 'Show Strength Indicator', 'powerplus-toolkit' ), 'type' => Controls_Manager::SWITCHER, 'default' => 'yes' ) );
		$this->add_control( 'success_redirect', array( 'label' => esc_html__( 'Success Redirect URL', 'powerplus-toolkit' ), 'type' => Controls_Manager::URL ) );
	...
	protected function render() {
		...
		if ( ! $is_preview && ( '' === $key || '' === $login ) ) {
			echo '<div class="pkwt-form-wrap"><p>' . esc_html__( 'Invalid password reset link.', 'powerplus-toolkit' ) . '</p></div>';
			return;
		}
